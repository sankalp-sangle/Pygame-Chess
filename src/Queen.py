class Queen:
    ...
    def validate(self, newX, newY, board):
        ...
        # Whatever is invalid for a queen is the intersection of whatever is invalid for bishop and whatever is invalid for rook
        if (newX != self.x and newY != self.y) and (abs(newX - self.x) != abs(newY - self.y)):
            return False
